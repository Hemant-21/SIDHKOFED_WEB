...
export interface CategoryCardDef {
  ...
  icon: ReactNode;
  /** i18n key for the card title. Ignored when `title` is set. */
  titleKey: string;
  /** i18n key for the card description. Ignored when `description` is set. */
        ...
        >
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold leading-tight text-foreground">
              {title ? pickText(title.en, title.hi, language) : t(titleKey)}
            </p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              {description ? pickText(description.en, description.hi, language) : t(descriptionKey)}
            </p>
          </div>
          <ArrowRight
